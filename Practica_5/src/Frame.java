...
public class Frame extends JFrame {
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				while(cutxt.hasMoreTokens()){
					xrel = 1;
					finali = analizador(cutxt.nextToken());
					if(finali.compareTo("END")==0){
						break;
					}
				}
				imprimir();
	...
	public String analizador(String strTexto){
		char charval[];
		String Intr = "";
		...
		for(x=1; x < strTexto.length(); x++ ){		
			if(charval[x]==';'){
				...
				x=strTexto.length();
			  Intr = Cortador(subusado);
			 
			}	
		}
		if (x==strTexto.length()){
			Intr = Cortador(strTexto);
		}
		return Intr;
	}
	...
	public String Cortador(String cadena){
		char charval[];
		char codver[];
		...
		if (cuentatokens==2){
			if(charval[0]==' '||charval[0]==';'){
				...
				if(Instr.compareTo(ORG)!=0 && Instr.compareTo(EQU)!=0 && Instr.compareTo(END)!=0 )
				{	
					//System.out.println("Analizando el Operador :"+Oper);
					Typex = AnaOper (Oper);
					//System.out.println("Operador :"+Typex	);
					String IMM = "IMM";
		...
		if (Instr.compareTo(EQU)==0 || Instr.compareTo(ORG)==0){
			agregarZapatos(Etiqueta, Instr, Oper, Dirr, LOCAN, operi);
		}
		else{
			agregarZapatos(Etiqueta, Instr, Oper, Dirr, LOCAN, operi);
		}
	...
	//
	void agregarZapatos (String Etiqueta, String Instr, String Oper, String Dirr, String CLocal, String Xopero) {
		lista.add(new Alojamiento (Etiqueta, Instr, Oper, Dirr, CLocal, Xopero ));
	}
	//
