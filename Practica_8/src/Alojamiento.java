...
public class Alojamiento {
	...
	String wracu (String gcConb,String Acu, String operx, String gdirr){
		...
		if(gdirr.compareTo("IDX")==0){
			Cadenatxt x = new Cadenatxt();
			System.out.println("IDXXXXXXXXXXXX");
			System.out.println("OPER "+Topoperandor);
			String v =x.analizaidx(Topoperandor);
			
			System.out.println("V "+v);
			
			int topo = Integer.parseInt(v,2);
			v = Integer.toHexString(topo);
			if(v.length()<2)
				v ="0"+v;
			int xbl= v.length();
			if(xbl>2){
				v = v.substring(xbl-2,xbl);
			}
			a1= acucut.nextToken();
			a2= acucut.nextToken();
